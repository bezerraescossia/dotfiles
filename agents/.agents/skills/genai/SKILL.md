...
| Current status | Do this | Then |
|---|---|---|
| `todo` | **Plan.** Read the task's *Plan* section and every approved artifact it builds on, and investigate enough to draft concretely (the repo, the real options, the constraints). **Where the draft would pick one of several reasonable approaches with a real difference in consequence** — would a second engineer reasonably have picked differently here? — **surface it before finalizing**: put every such choice to the user in one `AskUserQuestion` round, your recommendation marked on each. If the user defers ("you decide"), proceed with the recommendation and note the deferral. Then write `artifacts/<phase>/<id>-plan.md`: what will be built, where in the repo, how it will be tested, what you need from the user (access, credentials, a sample, a decision), and a **Choices made** section — each surfaced choice, what was picked, and why, including any explicit deference. If something needed is missing, ask before planning around it. | `set <id> prepared`. Show the plan and ask for approval before building. |
...
9. **Never bake an unasked implementation choice into an AI task's plan.** Where a second engineer could reasonably have picked differently, with a real consequence, surface it — recommend, but let the user decide or explicitly defer — before the plan is finalized.
...
| Silent implementation choice | An AI task's plan quietly picks one of several reasonable approaches (a parser, a chunking strategy, an IaC tool) with no record of the alternative or why. | Surface it in one `AskUserQuestion` round with a recommendation before finalizing the plan. Note the choice, and any explicit deference, in the plan's **Choices made** section. |
